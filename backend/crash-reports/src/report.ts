...
const EVENTS = ["HOME_OPENED", "SETTINGS_OPENED", "OVERLAY_CREATED", "OVERLAY_SHOWN", "OVERLAY_HIDDEN", "OVERLAY_DESTROYED", "ROTATION_CHANGED", "TAB_SHOWN", "TAB_HIDDEN", "VIEW_ATTACHED", "VIEW_DETACHED", "SCENARIO_START_REQUESTED", "SCENARIO_STOP_REQUESTED", "ACTION_FAILED", "CLICK", "SWIPE", "PAUSE", "INTENT", "TOGGLE_EVENT", "CHANGE_COUNTER", "EXTERNAL_ACTION", "NOTIFICATION", "SYSTEM_ACTION", "SET_TEXT", "ACTIVITY_RESUMED", "ACTIVITY_PAUSED", "DROPDOWN_OPENED", "DROPDOWN_CLOSED"];
...
export function validateReport(input: unknown): Report {
  const root = object(input, ["schemaVersion", "redactionVersion", "reportId", "build", "device",
    "mainThread", "crash", "recentOperations", "redactionCount", "truncated"], ["recentErrors"]);
  requireValue([1, 2].includes(root.schemaVersion as number) && root.redactionVersion === 1);
  const v2 = root.schemaVersion === 2;
  requireValue(v2 || !Object.hasOwn(root, "recentErrors"));
  ...
  const b = object(root.build, ["versionName", "versionCode", "flavor", "buildType"], v2 ? ["mappingId"] : []);
  optional(b, "mappingId", v => { string(v, 64); requireValue(/^[a-f0-9]{7,64}$/.test(v)); });
  ...
  for (const entry of array(root.recentOperations, v2 ? 128 : 50)) {
    const e = object(entry, ["event", "millisecondsBeforeCrash"], v2 ? ["component", "count", "state", "attached"] : []);
    requireValue((v2 ? EVENTS : ["HOME_OPENED", "SETTINGS_OPENED"]).includes(e.event as string));
    optional(e, "component", v => { string(v, 256); requireValue(/^[A-Za-z0-9_.$]+$/.test(v)); });
    optional(e, "count", v => integer(v, 1));
    optional(e, "state", v => integer(v, 0, 4));
    optional(e, "attached", boolean);
    integer(e.millisecondsBeforeCrash, 0, Number.MAX_SAFE_INTEGER);
  }
  if (root.recentErrors !== undefined) for (const value of array(root.recentErrors, 4)) {
    const e = object(value, ["type", "millisecondsBeforeCrash", "frames"], ["message"]);
    diagnosticString(e.type, 256); optional(e, "message", v => diagnosticString(v, 2048));
    integer(e.millisecondsBeforeCrash, 0, Number.MAX_SAFE_INTEGER);
    for (const value of array(e.frames, 16)) {
      const f = object(value, ["class", "method", "line", "native"], ["file"]);
      diagnosticString(f.class, 256); diagnosticString(f.method, 256);
      optional(f, "file", v => diagnosticString(v, 128)); integer(f.line); boolean(f.native);
    }
  }
  ...
  while (pending.length) {
    ...
    const e = object(value, [], ["type", "message", "frames", "cause", "suppressed", "detection", "nativeExit", "truncated", ...(v2 ? ["nativeTrace"] : [])]);
    ...
    optional(e, "message", v => diagnosticString(v, v2 ? 8192 : 2048));
    ...
    if (e.nativeExit !== undefined) {
      ...
      if (e.nativeTrace !== undefined) {
        const t = object(e.nativeTrace, ["signal", "code", "frames"]);
        integer(t.signal); integer(t.code);
        for (const frame of array(t.frames, 64)) {
          const f = object(frame, ["library", "relativePc", "functionOffset"], ["function", "buildId"]);
          string(f.library, 128);
          requireValue(f.library === "unknown" || /^[A-Za-z0-9_.+-]+\.(so|apk|oat|odex)$/.test(f.library));
          for (const key of ["relativePc", "functionOffset"]) { string(f[key], 16); requireValue(/^[a-f0-9]+$/.test(f[key] as string)); }
          optional(f, "function", v => diagnosticString(v, 256));
          optional(f, "buildId", v => { string(v, 128); requireValue(/^[a-fA-F0-9]+$/.test(v)); });
        }
      }
    ...
    } else requireValue(e.type !== "android.native_crash" && !Object.hasOwn(e, "nativeTrace"));
  }
